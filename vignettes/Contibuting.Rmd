...
## Overview
There are two steps you must complete the first time you want to contribute, or 
when you change computer:

a. Generate a personal access token (PAT) to allow R access to the GitHub API.
...
When working on a contribution, do so on a seperate feature branch. It is best practice to never make commits to the default branch of a fork. To create your feature branch, run
```{r, eval=FALSE}
...
In contrast to vignettes, R functions have much stricter formal [requirements](https://r-pkgs.org/code.html). Use R functions when you need to automate a common task, such as a data management task. There are a lot of considerations to writing a good R function, and it will generally be a bigger commitment than writing a vignette. A function needs good documentation, explaining what it does and how to use it. It should also have comprehensive and descriptive error handling. The goal is not to make the sleakest, fastest and most efficient functions, but rather implementing functionalities tailored to our specific needs. With that said, writing functions is not a one and done proces, and we encourage you to improve functions added to the package over time.     
For a deep dive into writing packages, check out [Hadley's R package book](https://r-pkgs.org/).
