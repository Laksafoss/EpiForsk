...
#'   the chi-square test for two-way tables (`var2` is specified). If `TRUE`, the
#'   table will include the chi-square p-value as well as the chi-square
...
freq_function <- function(
  ...
  if (!is.null(values_to_remove)) {
    if (!inherits(values_to_remove, "character")) {
      stop("When 'values_to_remove' is specified it must be a character vector.")
    }
  }
  ...
  if (include_NA  ==  TRUE) {
    ...
    func_table5 <- func_table4 %>% dplyr::filter(!is.na(Variable1))
  } else if (include_NA == FALSE & !is.null(var2)) {
    func_table5 <- func_table4 %>% dplyr::filter(!is.na(Variable1) & !is.na(Variable2))
  }
  ...
  if (is.null(values_to_remove)) {
    ...
    func_table6 <- func_table5 %>% dplyr::filter(!Variable1 %in% values_to_remove)
  } else if (!is.null(var2)) {
    func_table6 <- func_table5 %>%
      dplyr::filter(!Variable1 %in% values_to_remove & !Variable2 %in% values_to_remove)
  }
  # else if (is.null(var2)) {func_table6 <- subset(func_table5, !substr(Variable1,1,3) %in% values_to_remove)}
  # else if (!is.null(var2)) {func_table6 <- subset(func_table5, !substr(Variable1,1,3) %in% values_to_remove &
  #                                                   !substr(Variable2,1,3) %in% values_to_remove)}
  #Change all factor variables in data to character variables (basically it will be by-variables who are changed)
  ...
  if (is.null(by_vars) & is.null(var2)) {
    ...
    func_table7 <- func_table6 %>%
      ...
      dplyr::summarize(
        n = dplyr::n(),
        n_weighted = sum(weight_used, na.rm = TRUE),
        .groups = 'drop'
      ) %>%
      dplyr::mutate(Func_var = str_sub(deparse(quote({{ var1 }})), 2)) %>%
      dplyr::select(Func_var, Variable = Variable1, n, n_weighted)

    #Summation within variables
    func_table8 <- func_table7 %>%
      dplyr::summarize(
        n_total = sum(n),
        n_weighted_total = sum(n_weighted, na.rm = TRUE)
      ) %>%
      dplyr::mutate(
        Func_var = str_sub(deparse(quote({{ var1 }})),2),
        Variable2 = "Total"
      ) %>%
      dplyr::select(
        Func_var,
    ...
    func_table9 <- bind_rows(func_table7,func_table8)
    ...
    func_table10 <- many_merge(
      ...
      dplyr::mutate(
        Column_pct = ((n_weighted/n_weighted.2)*100),
        Freq_col_pct = paste0(
          n,
          " (",
          sprintf(paste0("%.",number_decimals,"f"),Column_pct),
          "%)"
        ),
        Freqw_col_pct = paste0(
          sprintf(paste0("%.",number_decimals,"f"),n_weighted),
          " (",
          sprintf(paste0("%.",number_decimals,"f"),Column_pct),
          "%)"
        )
    ...
    if(output == "all") {
      ...
      func_table11 <- dplyr::select(func_table10, -starts_with("Freq"))
    } else if (output == "col") {
    ...
    func_table10 <- many_merge(
      ...
      #sortnr = case_when(Variable == "Total" ~ as.character("Z"), TRUE ~ as.character("A"))) %>%
      #dplyr::select(all_of({{ by_vars }}), Level = Variable, N = n, N_weighted = n_weighted, Column_pct, Freq_col_pct, Freqw_col_pct, sortnr) %>%
      dplyr::select(
        all_of({{ by_vars }}),
        Level = Variable,
        N = n,
      ...
      arrange_at(append({{ by_vars }}, "Level"))
    #arrange_at(append({{ by_vars }}, "sortnr", "Level")) #%>%
    #dplyr::select(-sortnr)

    #Getting number of by-vars (needed to be able to change the correct name below)
    ...
    func_table7 <- func_table6 %>%
      dplyr::group_by(Variable1, Variable2) %>%
      dplyr::summarize(
        n = dplyr::n(),
        n_weighted = sum(weight_used, na.rm = TRUE),
        .groups = 'drop'
      ...
      dplyr::mutate(
        Func_var1 = gsub('"', "", str_sub(deparse(quote({{ var1 }})), 2)),
        Func_var2 = gsub('"', "", str_sub(deparse(quote({{ var2 }})), 2))
      ) %>%
      dplyr::select(Func_var1, Func_var2, Variable1, Variable2, n, n_weighted)
    ...
    func_table8 <- func_table7 %>%
      ...
      dplyr::summarize(
        n = sum(n),
        n_weighted = sum(n_weighted, na.rm = TRUE),
        .groups = 'drop'
      ) %>%
      dplyr::mutate(
        Func_var1 = gsub('"', "", str_sub(deparse(quote({{ var1 }})), 2)),
      ...
      dplyr::select(Func_var1, Variable1, Variable2 = Variable3, n, n_weighted)
    ...
    func_table9 <- func_table7 %>%
      ...
      dplyr::select(Func_var2, Variable1 = Variable3, Variable2, n, n_weighted)
    ...
    #Combining all numbers so percent etc. can be calculated in several directions
    ...
    func_table14 <- func_table13 %>%
      dplyr::mutate(
        Total_pct = ((n_weighted / Total_n_weighted) * 100),
        Row_pct = ((n_weighted / Variable1_level_total_weighted) * 100),
        Column_pct = ((n_weighted / Variable2_level_total_weighted) * 100)
      ) %>%
      dplyr::select(
        Func_var1,
        Func_var2,
    ...
    func_table15 <- func_table13 %>%
      dplyr::mutate(
        Variable3 = "Total",
        n = Variable1_level_total,
        Total_pct = ((Variable1_level_total_weighted / Total_n_weighted) * 100),
        Column_pct = ((Variable1_level_total_weighted / Total_n_weighted) * 100),
        Row_pct = ((Variable1_level_total_weighted / Variable1_level_total_weighted) * 100)
      ) %>%
      dplyr::select(
        Func_var1,
        Func_var2,
    ...
    func_table16 <- func_table13 %>%
      dplyr::mutate(
        Variable3 = "Total",
        n = Variable2_level_total,
        Total_pct = ((Variable2_level_total_weighted / Total_n_weighted) * 100),
        Column_pct = ((Variable2_level_total_weighted / Variable2_level_total_weighted) * 100),
        Row_pct = ((Variable2_level_total_weighted / Total_n_weighted) * 100)
      ) %>%
      dplyr::select(
        Func_var1,
        Func_var2,
    ...
    func_table17 <- func_table13 %>%
      dplyr::mutate(
        Variable3 = "Total",
        n = Total_n,
        Total_pct = ((Total_n_weighted / Total_n_weighted) * 100),
        Column_pct = ((Total_n_weighted / Total_n_weighted) * 100),
        Row_pct = ((Total_n_weighted / Total_n_weighted) * 100)
      ) %>%
      dplyr::select(
        Func_var1,
        Func_var2,
    ...
    func_table18 <- distinct(func_table15)
    func_table19 <- distinct(func_table16)
    func_table20 <- distinct(func_table17)

    #Combine all n/percentages in one table
    func_table21 <- bind_rows(
      func_table14,
      func_table18,
      func_table19,
      func_table20
      ...
      dplyr::mutate(
        ...
        Freqw_col_pct = paste0(
          sprintf(paste0("%.",number_decimals,"f"),Weighted_N),
          " (",
          sprintf(paste0("%.",number_decimals,"f"),Column_pct),
          "%)"
        ),
        Freq_row_pct = paste0(
          N,
          " (",
          sprintf(paste0("%.",number_decimals,"f"),Row_pct),
          "%)"
        ),
        ...
        Freq_total_pct = paste0(
          N,
          " (",
          sprintf(paste0("%.",number_decimals,"f"),Total_pct),
          "%)"
        ),
    ...
    #func_table22 <- func_table21 #%>%
    # pivot_wider(names_from = c(Func_var2,Variable2),
    #             values_from = c(N, Weighted_N, Row_pct ,Column_pct, Total_pct,
    #                           Freq_total_pct, Freqw_total_pct, Freq_row_pct, Freqw_row_pct, Freq_col_pct, Freqw_col_pct),
    #             values_fill = 0)
    ...
    if (chisquare == TRUE){
      ...
      func_table23 <- func_table13 %>%
        dplyr::mutate(
          expected = ((Variable1_level_total_weighted / Total_n_weighted) * Variable2_level_total_weighted),
          observed = n_weighted,
          cell_chi = (((observed - expected)^2) / expected)
        )
      #return(func_table23)

      #Summing up the total chi-square test statistic
      func_table24 <- func_table23 %>%
        dplyr::summarize(cell_chi_total = sum(cell_chi, na.rm = TRUE))
      #return(func_table24)
      ...
      #return(chi_degree_total)

      chi_p_prp <- bind_cols(func_table24, chi_degree_freedom)
      ...
      #colnames(chi_p)[length(chi_p)] <- Orig_var1_name
      #return(chi_p)
    } else {
    ...
    if(output == "all") {
      ...
      func_table26 <- dplyr::select(func_table25, -starts_with("Freq"))
    } else if (output == "col"){
      func_table26 <- dplyr::select(
        func_table25,
        -starts_with("N_"),
        -starts_with("Weighted_N_"),
        -starts_with("Row_pct_"),
        -starts_with("Column_pct_"),
        -starts_with("Total_pct_"),
        -starts_with("Freq_total_"),
        -starts_with("Freqw_total_"),
        -starts_with("Freq_row_"),
        -starts_with("Freqw_row_"),
        -starts_with("Freqw_col_")
      )
    } else if (output == "colw"){
      func_table26 <- dplyr::select(
        func_table25,
    ...
    func_table22 <- suppressMessages(
      ...
      arrange(dplyr::across(all_of({{ by_vars }})))
    #arrange_at(all_of({{ by_vars }})))
    ...
    if (chisquare == TRUE) {
      ...
      func_table24 <- func_table23 %>%
        dplyr::group_by_at({{ by_vars }}) %>%
        dplyr::summarize(
          cell_chi_total = sum(cell_chi, na.rm = TRUE),
          .groups = 'drop'
        )
      #return(func_table24)
      ...
      chi_degree1 <- func_table6 %>%
        dplyr::group_by_at({{ by_vars }}) %>%
        dplyr::summarize(chi_degree1 = n_distinct(Variable1), .groups = 'drop')
      chi_degree2 <- func_table6 %>%
        dplyr::group_by_at({{ by_vars }}) %>%
        dplyr::summarize(chi_degree2 = n_distinct(Variable2), .groups = 'drop')
